...
import tech.alexchen.spring.ioc.config.BeansConfig;
import tech.alexchen.spring.ioc.bean.SysUserService;
...
class IocTest {
    ...
    void annotationConfig() {
        AnnotationConfigApplicationContext context =
                new AnnotationConfigApplicationContext("tech.alexchen.spring.ioc.bean");
        SysUserService userService = context.getBean("userService", SysUserService.class);
        List<SysUser> userList = userService.findUserList();
        userList.forEach(a ->
                System.out.println(a.getUsername())
